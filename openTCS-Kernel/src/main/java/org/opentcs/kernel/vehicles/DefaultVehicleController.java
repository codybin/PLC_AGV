...
if(commAdapter instanceof  KeCongCommAdapter)
{
((KeCongCommAdapter)commAdapter).setListPoints(vehicleService.fetchObjects(Point.class).stream().collect(Collectors.toList()));
}
    initialized = true;
  }
  ...
  private void allocateForNextCommand() {
    ...
    scheduler.allocate(this, pendingResources);//分配资源
